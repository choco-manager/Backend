...
namespace Backend.Tests.Unit.Modules.PriceTypes;

[TestFixture]
public class PriceTypesModuleTests {
  ...
  [Test(Description =
    "Method to get list of price types should return status code 200 and list from 5 predefined price types")]
  public async Task GetPriceTypes__Returns200() {
    // Act
    var response = await _client.GetAsync("/api/priceTypes");
    var movementStatuses = await response.Content.ReadFromJsonAsync<List<PriceType>>();

    // Assert
    response.StatusCode.Should().Be(HttpStatusCode.OK);
    movementStatuses.Should().NotBeNull();
    movementStatuses?.Count.Should().Be(2);
  }
}