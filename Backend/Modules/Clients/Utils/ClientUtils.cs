...
namespace Backend.Modules.Clients.Utils;

public class ClientUtils {
  public List<Guid> GetDeltaOfGuidLists(List<Guid> oldList, List<Guid> newList) {
    var secondHashSet = new HashSet<Guid>(newList);
    var deltaList = oldList.Where(item => secondHashSet.Contains(item)).ToList();
    deltaList.AddRange(newList.Where(item => !oldList.Contains(item)));
