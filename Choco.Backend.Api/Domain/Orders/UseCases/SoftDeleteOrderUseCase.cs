...
public class SoftDeleteOrderUseCase(AppDbContext db) : IUseCase<IdModel, EmptyResponse>
{
    public async Task<Result<EmptyResponse>> Execute(IdModel req, CancellationToken ct = default)
    {
        var order = await db.Orders.Where(e => e.Id == req.Id).FirstOrDefaultAsync(ct);
